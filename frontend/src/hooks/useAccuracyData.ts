...
export function useAccuracyData() {
  ...
  const [backtests, setBacktests] = useState<Map<string, BacktestResponse>>(
    new Map()
  );
    ...
    try {
      ...
      const results = await Promise.allSettled(
        list.map((m) =>
          ...
          }>(`${API_BASE}/api/signals/models/${m.modelId}/backtest`).then(
            (r) => ({ modelId: m.modelId, data: r.data })
          )
        )
      );
  ...
  const overallAccuracy = useMemo(() => {
    const valid = models.filter(
      (m) => m.avgMape !== null && m.avgMape !== undefined
    );
