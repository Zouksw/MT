...
import { isAuthError, isNetworkError, useErrorHandler } from "../useErrorHandler";
...
describe("useErrorHandler", () => {
	...
	describe("parseErrorMessage", () => {
		...
		it("should extract error property from object", () => {
			const { result } = renderHook(() => useErrorHandler());
			expect(result.current.parseErrorMessage({ error: "API error occurred" })).toBe(
				"API error occurred",
			);
		});
		...
		it("should extract message property from object", () => {
			const { result } = renderHook(() => useErrorHandler());
			expect(result.current.parseErrorMessage({ message: "Message error occurred" })).toBe(
		...
		it("should extract error from response.data", () => {
			const { result } = renderHook(() => useErrorHandler());
			expect(
				result.current.parseErrorMessage({ response: { data: { error: "Response error" } } }),
			).toBe("Response error");
		});
		...
		it("should extract message from response.data", () => {
			const { result } = renderHook(() => useErrorHandler());
			expect(
				result.current.parseErrorMessage({ response: { data: { message: "Response message" } } }),
			).toBe("Response message");
		});

		it("should extract message from Error object", () => {
			const { result } = renderHook(() => useErrorHandler());
			expect(result.current.parseErrorMessage(new Error("Error object message"))).toBe(
		...
		it("should return default message for unknown error", () => {
			const { result } = renderHook(() => useErrorHandler());
			expect(result.current.parseErrorMessage({ unknown: "value" })).toBe(
