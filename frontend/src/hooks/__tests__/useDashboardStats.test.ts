  ...
  it('should fetch and parse stats successfully', async () => {
    mockUseRetryableFetch.mockImplementation((_key: any, _fetcher: any) => {
      ...
      const dataMap: any[] = [
        ...
        { total: 15, data: [{ severity: 'critical' }, { severity: 'high' }, { severity: 'medium' }, { severity: 'low' }] }, // alerts
      ...
      const dataByCallOrder: Record<number, any> = {
        ...
        1: { total: 25, data: [] },
        2: { total: 5, data: [] },
        3: { total: 15, data: [{ severity: 'critical' }, { severity: 'high' }, { severity: 'medium' }, { severity: 'low' }] },
        4: { data: [{ id: 1 }] },
        5: { data: [{ id: 1 }] },
      };
