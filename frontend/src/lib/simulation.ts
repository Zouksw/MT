...
export function useSimAccounts() {
  const { data, error, mutate } = useSWR<{ success: boolean; data: { accounts: Array<{
    id: string;
    name: string;
    initialBalance: number;
    currentBalance: number;
...
export function useSimOrders(accountId: string | null) {
  const { data, error } = useSWR<{ success: boolean; data: { orders: Array<Record<string, unknown>> } }>(
    accountId ? `/sim/accounts/${accountId}/orders` : null,
    fetcher,
...
export function useSimTrades(accountId: string | null) {
  const { data, error } = useSWR<{ success: boolean; data: { trades: Array<Record<string, unknown>> } }>(
    accountId ? `/sim/accounts/${accountId}/trades` : null,
    fetcher,
    { refreshInterval: 10000 },
  );
