...
/**
 * Shared form for creating and editing market-news articles.
 *
 * Extracted from create/page.tsx so the edit page can reuse the exact same
 * fields + validation + sanitization. The submit handler is caller-supplied
 * (create vs PATCH differ); an optional `initial` state seeds the form for
 * edit mode.
 */
...
export interface NewsFormProps {
	...
	/** Submit button label (e.g. "Publish Article" / "Save Changes"). */
	submitLabel: string;
	loadingLabel?: string;
	/** Performs the actual create/update. Receives the sanitized payload. */
	onSubmit: (payload: {
		title: string;
		summary: string;
		body: string;
...
export function NewsForm({ initial, submitLabel, loadingLabel, onSubmit }: NewsFormProps) {
	...
	const set = (field: keyof NewsFormState, value: string) => {
	...
	const handleSubmit = async (ev: React.FormEvent) => {
		...
		try {
			...
			toast.showError(err instanceof Error ? err.message : "Failed to save article");
		} finally {
			setSubmitting(false);
		}
	};
		...
		<form onSubmit={handleSubmit} className="max-w-2xl space-y-5">
			...
			<div className="flex items-center gap-3 pt-2">
				<Button type="submit" variant="primary" size="lg" isLoading={submitting}>
					{submitting ? (loadingLabel ?? "Saving…") : submitLabel}
				</Button>
				<Button type="button" variant="ghost" size="lg" onClick={() => router.push("/market-news")}>
					Cancel
				</Button>
			</div>
		</form>
