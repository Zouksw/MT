      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-1.5">
          {factors.map((f) => {
            ...
            return (
              <div key={f.id} className="flex items-center justify-between text-xs text-muted-foreground py-1 border-t border-muted">
                <div className="flex items-center gap-2">
                  <span>{new Date(f.date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                  {f.region && <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted">{f.region}</span>}
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-medium text-foreground">{Number(f.value).toLocaleString()} {f.unit}</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted/80">{sourceLabel}</span>
                </div>
              </div>
            );
          })}
