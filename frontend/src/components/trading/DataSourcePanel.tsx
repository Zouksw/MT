...
import { Database, CheckCircle, TrendingUp, BarChart3 } from "lucide-react";
import {
  Card,
  CardHeader,
  CardTitle,
...
export default function DataSourcePanel({ priceSources, factorSources, loading }: DataSourcePanelProps) {
  if (loading) {
    return (
      <Card>
        <CardHeader>
  ...
  if (priceSources.length === 0 && factorSources.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">No data sources available for this commodity.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          {factorSources.length > 0 && ` · ${factorSources.length} market factor source${factorSources.length !== 1 ? "s" : ""}`}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {/* Price data sources */}
        {priceSources.length > 0 && (
          <div className="space-y-3 mb-4">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Price Data</h4>
            {priceSources.map((src) => {
              const badge = tierBadge(src.priceCount);
              const BadgeIcon = badge.icon;
              return (
                <div key={src.id} className="flex items-center justify-between gap-3 p-2 rounded-lg bg-muted/50">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${sourceColors[src.id] || "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400"}`}>
                      {src.label}
                    </span>
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0">
                    ...
                    <span>{fmtDate(src.dateRange.from)} — {fmtDate(src.dateRange.to)}</span>
                  </div>
                </div>
              );
            })}
        ...
        {factorSources.length > 0 && (
          <div className="space-y-3">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Market Factors</h4>
            {factorSources.map((f, i) => (
              <div key={`${f.source}-${f.type}-${i}`} className="flex items-center justify-between gap-3 p-2 rounded-lg bg-muted/50">
                <div className="flex items-center gap-2 min-w-0">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${sourceColors[f.source] || "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400"}`}>
                    {f.label}
                  </span>
