...
function ChartSkeleton({ height }: { height: number }) {
  return (
    <div
      className="w-full rounded-lg animate-skeleton-pulse"
      style={{ height }}
    >
...
export default function ProfessionalChart({
  ...
  const seriesRef = useRef<Map<string, ISeriesApi<"Candlestick" | "Line" | "Histogram">>>(new Map());
  ...
  const buildChart = useCallback(() => {
    ...
    const chartOptions: DeepPartial<ChartOptions> = {
      layout: {
        background: { type: ColorType.Solid, color: isDark ? CHART_COLORS.bgDark : CHART_COLORS.bgLight },
  ...
  const setChartData = useCallback(() => {
    ...
    if (chartType === "candlestick") {
      chart.subscribeCrosshairMove((param: MouseEventParams) => {
        ...
        const candlePoint = param.seriesData.get(candleSeriesApi) as
          | LWCandlestickData
          | undefined;
        ...
        if (candlePoint && "open" in candlePoint) {
          ...
          setLegend({
            ...
            date: typeof timeStr === "string" ? timeStr : new Date((timeStr as number) * 1000).toISOString().slice(0, 10),
          });
        }
      });
    }
        ...
        <div className="absolute top-2 left-3 z-10 flex gap-3 text-xs font-mono">
          ...
          {legend.volume > 0 && (
            <span className="text-gray-500">
              V {legend.volume.toLocaleString()}
            </span>
          )}
        </div>
      )}
