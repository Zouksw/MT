...
function useAnimatedCounter(target: number, duration = 800) {
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) { prevTarget.current = target; return; }
...
function Sparkline({ data, color }: { data: number[]; color: string }) {
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className="mt-1" style={{ overflow: "visible" }} role="img" aria-label="Sparkline chart">
      ...
      <circle cx={points[points.length - 1].x} cy={points[points.length - 1].y} r={2} fill={color} opacity={0}
