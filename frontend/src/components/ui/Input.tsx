...
export const Input: React.FC<InputProps> = ({
	...
	const combinedClassName =
		`${baseStyles} ${error ? errorStyles : normalStyles} ${widthStyles} ${className}`.trim();

	return (
		<div className={fullWidth ? "w-full" : ""}>
			{label && (
				<label htmlFor={inputId} className="block text-body-sm font-medium text-foreground mb-1">
					{label}
				</label>
			)}
			<input id={inputId} className={combinedClassName} style={{ height: "40px" }} {...props} />
...
export const Textarea: React.FC<TextareaProps> = ({
	...
	return (
		<div className={fullWidth ? "w-full" : ""}>
			{label && (
				<label htmlFor={textareaId} className="block text-body-sm font-medium text-foreground mb-1">
					{label}
				</label>
			)}
			<textarea id={textareaId} className={combinedClassName} rows={4} {...props} />
			{error && <p className="mt-1 text-body-sm text-error">{error}</p>}
			{helperText && !error && (
				<p className="mt-1 text-body-sm text-muted-foreground">{helperText}</p>
			)}
