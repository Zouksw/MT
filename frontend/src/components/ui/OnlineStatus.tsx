...
import { useOnlineStatus, useOnlineStatusWithCallbacks } from "@/hooks/useOnlineStatus";
...
export const OnlineStatus: React.FC<OnlineStatusProps> = ({
	...
	if (mode === "badge") {
		return (
			<div className={`inline-flex items-center gap-2 cursor-pointer ${posClass}`}>
				...
				<span className="text-sm text-foreground">{isOnline ? onlineMessage : offlineMessage}</span>
				{isOnline ? (
	...
	if (mode === "icon") {
		return (
			<div
				className={`inline-flex items-center cursor-pointer ${posClass}`}
				title={isOnline ? "Connected to internet" : "Disconnected from internet"}
			>
				{isOnline ? (
					<Wifi className="text-emerald-500" style={{ width: size, height: size }} />
				) : (
					<WifiOff className="text-red-500" style={{ width: size, height: size }} />
				)}
			</div>
		);
	}
