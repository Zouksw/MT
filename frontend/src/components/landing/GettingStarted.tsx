...
const steps = [
  {
    ...
    description: "Sign up free and customize your commodity watchlist. Choose the markets you care about.",
  ...
  },
  {
    ...
    description: "Browse 55+ commodities with real-time prices, candlestick charts, and historical trends.",
    ...
    preview: (
      <div className="space-y-2 mt-3">
        {[
          ...
          <div key={c.name} className="flex items-center justify-between px-3 py-2 rounded-md bg-gray-50 dark:bg-gray-800/60">
            <span className="text-xs text-muted-foreground">{c.name}</span>
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono text-gray-900 dark:text-white tabular-nums">{c.price}</span>
              <span className={`text-xs font-mono tabular-nums ${c.up ? "text-green-600 dark:text-green-400" : "text-red-500"}`}>{c.chg}</span>
            </div>
          </div>
        ))}
      </div>
  ...
  },
  {
    ...
    description: "Configure which AI models to follow, set price alerts, and customize your dashboard.",
    ...
    preview: (
      <div className="mt-3 space-y-2">
        {[
          ...
          <div key={s.model} className="flex items-center gap-3 px-3 py-2 rounded-md bg-gray-50 dark:bg-gray-800/60">
            ...
            <span className={`text-xs font-mono tabular-nums ${s.signal === "Buy" ? "text-green-600 dark:text-green-400" : "text-amber-500"}`}>{s.signal} {s.conf}%</span>
          </div>
        ))}
      </div>
    ),
  },
  {
    number: 4,
    title: "Track & Decide",
    description: "Monitor AI-generated signals, compare model accuracy, and use multi-factor analysis.",
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        <div className="mb-12 md:mb-16 lg:mb-20 max-w-2xl">
          ...
          <MotionReveal delay={0.1}>
            <h2 className="font-display text-3xl font-semibold text-gray-900 dark:text-white md:text-4xl lg:text-5xl" style={{ letterSpacing: "-0.04em" }}>
