  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      const lines = [
        ...
        { data: DATA_STREAM.map((v) => v * 0.7 + 10), color: "rgba(184, 134, 11, 0.25)", width: 1.5 },
...
const features = [
  {
    ...
    description: "Track weather, exchange rates, tariffs, and shipping costs alongside commodity prices",
    span: "md:col-span-2",
  },
  {
    icon: <Lightning size={24} weight="duotone" />,
    title: "7 AI Signal Models",
    description: "ARIMA, HoltWinters, ExponentialSmoothing and more — generating buy/sell signals automatically",
    span: "",
  },
  {
          ...
          <div className="lg:col-span-3">
            ...
            <MotionReveal delay={0.2}>
              <p className="mt-6 max-w-xl text-lg font-medium text-muted-foreground leading-relaxed">
                Track commodity prices with AI-driven signals across weather, forex, tariffs, and shipping factors.{" "}
                <span className="font-mono text-primary">55+ commodities</span>,{" "}
            ...
            <MotionReveal delay={0.4}>
              <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-2 text-sm text-muted-foreground font-mono tabular-nums">
                <div className="flex items-center gap-2">
                  <span className="text-primary dark:text-primary font-semibold text-base">55+</span>
                ...
                <div className="w-px h-4 bg-muted" />
                <div className="flex items-center gap-2">
                  <span className="text-primary dark:text-primary font-semibold text-base">Real-time</span>
            ...
            <MotionReveal delay={0.3}>
              <div className="rounded-2xl bg-gray-950 p-4 text-white">
                ...
                <div className="mb-3 space-y-1.5">
                  {priceItems.map((item) => (
                    <div key={item.name} className="flex items-center justify-between px-3 py-2 rounded-md bg-white/5">
                      <span className="text-xs font-medium text-gray-300">{item.name}</span>
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-mono text-white tabular-nums">${item.price}</span>
                        <span className={`text-xs font-mono tabular-nums ${item.up ? "text-green-400" : "text-red-400"}`}>
                          {item.change}
                        </span>
                      </div>
                    </div>
