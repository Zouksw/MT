...
const features = [
  ...
  },
  {
    ...
    description:
      "Discover monthly price patterns and cross-commodity correlations.",
    ...
    <div className="mt-4 space-y-2">
      {correlationData.map((item) => (
        <div key={item.pair} className="flex items-center gap-3">
          ...
          <span className={`text-xs font-mono tabular-nums w-10 text-right ${item.value > 0 ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400"}`}>
...
function CommoditiesVisual() {
  ...
  return (
    <div className="mt-4 grid grid-cols-2 gap-2">
      {commodities.map((c) => (
        <div key={c.name} className="flex items-center justify-between px-3 py-2 rounded-md bg-gray-50 dark:bg-gray-700/30">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs font-mono tabular-nums ${c.up ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400"}`}>
              {c.change}
            </span>
          </div>
        </div>
...
export const Features: React.FC = () => {
  return (
    <section id="features" className="relative overflow-hidden bg-white dark:bg-[#0a0a0a] px-6 py-24 md:py-36 lg:py-48">
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        <div className="mb-12 md:mb-16 lg:mb-20 max-w-2xl">
          ...
          <MotionReveal delay={0.1}>
            <h2 className="font-display text-3xl font-semibold text-gray-900 dark:text-white md:text-4xl lg:text-5xl" style={{ letterSpacing: "-2.4px" }}>
              Everything you need
            </h2>
          </MotionReveal>
