        ...
        <div>
          <div className="rounded-xl overflow-hidden bg-card border">
            ...
            <div className="flex flex-col items-center -mt-10 px-6">
              ...
              <Tag color="primary" className="mb-4">{user?.roles?.[0] || "User"}</Tag>
              <div className="flex items-center gap-2 mb-4">
                <span className="h-2 w-2 rounded-full bg-green-500" />
                <span className="text-body-sm text-green-600 dark:text-green-400 font-medium">Active</span>
              </div>
              <Button variant="primary" className="w-full mb-6" onClick={() => router.push("/settings/profile")}>
                Edit Profile
              </Button>
            </div>
          </div>
        ...
        <div>
          <div className="bg-card border rounded-xl p-6 text-center">
            ...
            <div className="relative inline-flex items-center justify-center mb-4">
              <svg className="w-32 h-32 -rotate-90" viewBox="0 0 128 128" role="img" aria-label="Security score gauge">
                ...
                <circle cx="64" cy="64" r="54" fill="none" stroke="#B8860B" strokeWidth="8" strokeLinecap="round" strokeDasharray={`${securityScore * 3.39} 339`} className="transition-all duration-1000 ease-out" />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-display font-semibold text-foreground">{Math.round(securityScore)}</span>
            ...
            <div className="space-y-3 mb-6">
              {securityItems.map((item, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  ...
                  <span className={`text-body ${item.enabled ? "text-foreground" : "text-muted-foreground"}`}>{item.label}</span>
                  <Tag color={item.enabled ? "success" : "default"} className="ml-auto">{item.enabled ? "Enabled" : "Coming Soon"}</Tag>
                </div>
              ))}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Button variant="ghost" className="h-auto py-3" onClick={() => router.push("/apikeys")}>
                Manage API Keys
              </Button>
              <Button variant="ghost" className="h-auto py-3" onClick={() => router.push("/settings/profile")}>
                Change Password
              </Button>
            </div>
          </div>
      ...
      <div className="mt-6">
        <div className="bg-card border rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { title: "View Sessions", desc: "See active sessions", path: "/settings/sessions", color: "from-emerald-500 to-teal-400",
                icon: <Clock className="size-4" /> },
              { title: "Notifications", desc: "Configure alerts", path: "/settings/notifications", color: "from-primary to-primary-hover",
                icon: <Bell className="size-4" /> },
              { title: "API Keys", desc: "Manage access tokens", path: "/apikeys", color: "from-purple-500 to-indigo-400",
                icon: <KeyRound className="size-4" /> },
              { title: "Profile", desc: "Edit personal info", path: "/settings/profile", color: "from-primary to-blue-400",
