...
import {
  Database, RefreshCw, CheckCircle, XCircle, Clock, Activity,
...
function TierBadge({ tier }: { tier: string }) {
  ...
  return (
    <span className={`px-1.5 py-0.5 text-[10px] font-semibold rounded ${colors[tier] || "bg-gray-100 text-gray-600"}`}>
      T{tier}
    </span>
  );
}
...
export default function DataSourcesPage() {
  ...
  const loadHistory = async (sourceId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/market/sources/${sourceId}/history?limit=10`, { headers });
      const data = await res.json();
  ...
  const avgSuccessRate = freshness.length > 0
                    ...
                    {isExpanded && (
                      <div className="px-4 py-3 bg-muted/30 border-t border-border">
                        ...
                        {historyLogs.length > 0 ? (
                          ...
                          <p className="text-xs text-muted-foreground">No ingestion history available</p>
                        )}
                      </div>
                    )}
                  </div>
