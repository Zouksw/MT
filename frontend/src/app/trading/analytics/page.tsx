					...
					<CardBody>
						<div className="text-center py-8">
							...
							<p className="text-muted-foreground">
								Create a backtest account first to see analytics
							</p>
						</div>
					</CardBody>
				</Card>
...
function RiskDashboard({ accounts }: { accounts: Array<{ id: string; name: string }> }) {
	...
	}>(selectedId ? `/analytics/risk/${selectedId}` : null, fetcher);
			...
			{isLoading ? (
				...
				<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
					<StatCard
						...
						color={risk.sharpe > 1 ? "green" : risk.sharpe > 0 ? "amber" : "red"}
					/>
					<StatCard
						title="Sortino Ratio"
						value={risk.sortino.toFixed(2)}
						color={risk.sortino > 1 ? "green" : risk.sortino > 0 ? "amber" : "red"}
					/>
					<StatCard
						title="Max Drawdown"
					...
					<StatCard
						...
						color={risk.winRate > 0.5 ? "green" : "red"}
					/>
					<StatCard
						title="Profit Factor"
						value={risk.profitFactor.toFixed(2)}
						color={risk.profitFactor > 1 ? "green" : "red"}
					/>
					<StatCard
