...
import { Key, Copy, RefreshCw, Pencil, Trash2, Eye, Shield, Home } from "lucide-react";
...
const usageLogColumns = [
  ...
  },
  {
    ...
    render: (status: number) => {
      const color = status >= 200 && status < 300 ? "success" : status >= 300 && status < 400 ? "warning" : "error";
...
export default function ApiKeyDetailPage({ params }: { params: Promise<ApiKeyDetailParams> }) {
  ...
  const handleRegenerate = () => {
    setConfirmModal({
      ...
      message: "Regenerating this API key will invalidate the current key immediately. Any existing integrations using this key will stop working until they are updated with the new key.",
  ...
  const handleRevoke = () => {
    setConfirmModal({
      ...
      message: "Revoking this API key will permanently disable it. Any applications using this key will lose access immediately.",
      confirmLabel: "Revoke",
      onConfirm: async () => {
        try {
    ...
    <div className="min-h-screen bg-muted p-4 md:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="ghost" size="sm" icon={<Copy className="size-3.5" />} onClick={handleCopyKey}>
            {!isMobile && "Copy Key"}
          </Button>
          <Button variant="ghost" size="sm" icon={<RefreshCw className="size-3.5" />} onClick={handleRegenerate}>
            {!isMobile && "Regenerate"}
          </Button>
          ...
          <Button variant="danger" size="sm" icon={<Trash2 className="size-3.5" />} onClick={handleRevoke}>
            {!isMobile && "Revoke"}
          </Button>
        </div>
      </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between w-full">
              ...
              <a href={`/apikeys/edit/${apiKey.id}`} className="text-sm text-primary hover:underline">Edit</a>
            </div>
          </CardHeader>
          <CardBody>
            {apiKey.permissions && apiKey.permissions.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {apiKey.permissions.map((permission) => (
                  <Tag key={permission} color="info">{permission}</Tag>
                ))}
              </div>
            ) : (
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between w-full">
              ...
              <span className="text-gray-400"><Shield className="size-3.5" /></span>
            </div>
          </CardHeader>
          <CardBody>
            <div className="flex flex-col gap-3">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-1 mt-1">
                    {apiKey.ipWhitelist.map((ip, index) => (
                      <Tag key={index} color="success">{ip}</Tag>
                    ))}
                  </div>
                </div>
              )}
              {apiKey.expiresAt && (
                <div>
                  ...
                  <div className={`text-sm mt-1 ${isExpired ? "text-red-600 dark:text-red-400" : "text-muted-foreground"}`}>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between w-full">
              ...
              <Button variant="ghost" size="sm" onClick={fetchUsageLogs}>Refresh</Button>
            </div>
          </CardHeader>
          <CardBody>
            {usageLogs.length === 0 ? (
              <p className="text-sm text-muted-foreground p-6">No usage logs available for this API key.</p>
            ) : (
              <Table<ApiUsageLog>
        ...
        <Card>
          ...
          <CardBody>
            <div className="flex flex-wrap gap-3">
              ...
              <Button variant="ghost" icon={<RefreshCw className="size-3.5" />} onClick={handleRegenerate}>
                Regenerate Key
              </Button>
              <Button variant="ghost" icon={<Eye className="size-3.5" />} onClick={() => router.push("/docs/api")}>
                View API Docs
              </Button>
            </div>
          </CardBody>
