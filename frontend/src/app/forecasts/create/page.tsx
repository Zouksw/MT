...
const AI_NODE_ALGORITHMS = [
  {
    ...
    features: ["Suitable for stationary time series", "Fast training", "Accurate short-term predictions"],
    hyperparameters: [
  ...
  },
  {
    ...
    features: ["Long-term dependency capture", "Complex pattern recognition", "Requires pretraining"],
    requiresWeights: true,
    hyperparameters: [
      { name: "model_path", label: "Model weights path", type: "string", placeholder: "/path/to/timer_xl_weights.pth" },
    ],
  },
  {
    value: "sundial",
    ...
    requiresWeights: true,
    hyperparameters: [
      { name: "model_path", label: "Model weights path", type: "string", placeholder: "/path/to/sundial_weights.pth" },
    ],
  },
];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/iotdb/ai/predict`,
        {
          method: "POST",
      ...
      <div className="mx-auto max-w-[1000px]">
        ...
        <div className="bg-card rounded-lg shadow-sm border border p-6 mb-6">
          ...
          <Alert variant="info">
            <div>
              <p className="mb-2">
                <strong>Directly call AI Node native models</strong> -- no pre-training required. Select time series data, algorithm, and parameters to generate predictions.
              </p>
              <ul className="list-disc pl-5 space-y-1 text-sm">
                <li><strong>Horizon:</strong> Number of time points to predict</li>
                <li><strong>Confidence Level:</strong> Confidence interval (0-1)</li>
                <li><strong>Algorithm:</strong> Choose an AI Node built-in algorithm</li>
                <li><strong>Hyperparameters:</strong> Optional model tuning parameters</li>
              </ul>
            </div>
          </Alert>
        </div>
                    ...
                    {algorithmInfo.hyperparameters.map((param) => {
                      ...
                      return isString ? (
                        ...
                        <Input
                          ...
                          value={hyperparameters[param.name] ?? ("default" in param ? String(param.default) : "")}
                          onChange={(e) => handleHyperparamChange(param.name, e.target.value)}
                          fullWidth
                          helperText={"default" in param ? `Default: ${param.default}` : undefined}
                        />
