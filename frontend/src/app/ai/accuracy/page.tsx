...
import { Target, Trophy, BarChart3, CircleCheck, TrendingUp, TrendingDown, Minus } from "lucide-react";
...
const columns = [
  {
    ...
    render: (_: string, record: ModelWithBacktest) => (
      <Link href={`/ai/accuracy/${record.modelId}`} className="font-medium text-foreground hover:text-primary transition-colors">
        {record.displayName}
      </Link>
    ),
  },
  ...
  },
  {
    ...
    align: "right" as const,
    render: (value: number) => <span className="text-sm text-muted-foreground">{value.toLocaleString()}</span>,
  },
  {
    key: "verifiedCount",
    title: "Verified",
