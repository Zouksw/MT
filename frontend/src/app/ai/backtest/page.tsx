...
export default function BacktestPage() {
    ...
    },
    {
      ...
      render: (_: unknown, record: BacktestWindow) => (
        <span className={`text-sm font-medium ${
          record.trend === "improving" ? "text-green-600 dark:text-green-400" :
          record.trend === "degrading" ? "text-red-600 dark:text-red-400" :
  ...
  return (
    <PageContainer>
      ...
      {error && (
        <ErrorDisplay error={error} retry={() => setSelectedModel(selectedModel)} context="backtest data" />
      )}
        ...
        {backtest && (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <StatCard
                title="Best MAPE"
                value={bestWindow?.mape !== null && bestWindow?.mape !== undefined ? Number(bestWindow.mape.toFixed(2)) : "--"}
                ...
                <div className="space-y-3">
                  {(backtest.windows || []).map((w) => (
                    <div key={w.window} className="flex items-center gap-4">
                        ...
                        {w.mape !== null && (
                          <div
                            className={`h-full rounded-md transition-all duration-500 ${
                              w.mape < 5 ? "bg-green-500/80 dark:bg-green-600/80" :
                              w.mape < 10 ? "bg-primary/80" :
                      ...
                      <span className={`text-sm font-mono font-medium w-20 text-right ${mapeColor(w.mape)}`}>
                        {formatMape(w.mape)}
                      </span>
                    </div>
                  ))}
