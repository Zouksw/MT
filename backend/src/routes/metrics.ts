...
router.get('/', (req: Request, res: Response) => {
  ...
  const avgResponseTime = totalRequestCount > 0
...
router.post('/web-vitals', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[METRICS] Failed to store web vital', { error: (err as Error).message });
...
router.get('/web-vitals', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[METRICS] Failed to get web vitals summary', { error: (err as Error).message });
...
router.get('/web-vitals/history', async (req: Request, res: Response) => {
  try {
    ...
    const parsed = entries
      .map((entry) => {
        try {
          return JSON.parse(entry) as { value: number; path: string; timestamp: number };
        } catch {
          return null;
        }
      })
    ...
    logger.error('[METRICS] Failed to get web vitals history', { error: (err as Error).message });
...
router.get('/api-latency', (req: Request, res: Response) => {
  ...
  const overallAvg = globalDurations.length > 0
...
router.get('/summary', async (req: Request, res: Response) => {
  try {
    ...
    for (const name of ['LCP', 'FID', 'CLS'] as const) {
      ...
      webVitals[name.toLowerCase()] = { avg: parseFloat(avg.toFixed(2)), count };
    }
    ...
    const apiLatencyAvg = globalDurations.length > 0
      ? parseFloat((globalDurations.reduce((s, v) => s + v, 0) / globalDurations.length).toFixed(2))
      : 0;
    ...
    const errorRate = totalRequestCount > 0
    ...
    logger.error('[METRICS] Failed to get summary', { error: (err as Error).message });
