...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const prices = await prisma.beefCutPrice.findMany({
      ...
      orderBy: { date: 'desc' },
      take: 500,
      include: { factory: { select: { code: true, name: true, country: true } } },
    });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const spreads: Record<string, Record<string, { min: number; max: number; avg: number; count: number }>> = {};
    for (const p of prices) {
      const key = p.cutCode;
      if (!spreads[key]) spreads[key] = {};
      const sourceKey = `${p.source} (${p.factory?.country || 'unknown'})`;
      if (!spreads[key][sourceKey]) {
        spreads[key][sourceKey] = { min: p.price, max: p.price, avg: p.price, count: 1 };
      } else {
