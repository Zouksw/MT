  ...
  asyncHandler(async (req: AuthRequest, res) => {
    const watchlists = await prisma.watchlist.findMany({
      ...
      include: {
        items: {
          include: {
            commodity: { select: { id: true, slug: true, name: true, nameCn: true, category: true, unit: true } },
          },
    ...
    const latestPrices = commodityIds.length > 0
...
router.patch(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    const existing = await prisma.watchlist.findUnique({ where: { id: req.params.id } });
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    const existing = await prisma.watchlistItem.findUnique({
      where: { watchlistId_commodityId: { watchlistId: req.params.id, commodityId } },
    });
    if (existing) {
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    const item = await prisma.watchlistItem.findUnique({
      where: { watchlistId_commodityId: { watchlistId: req.params.id, commodityId: req.params.commodityId } },
    });
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    const watchlist = await prisma.watchlist.findUnique({
      ...
      include: {
        items: {
          include: {
            commodity: { select: { id: true, slug: true, name: true, nameCn: true, unit: true } },
          },
        },
      },
    });
    ...
    const recentPrices = commodityIds.length > 0
      ? await prisma.$queryRaw<Array<{ commodity_id: string; close: number; date: Date; rn: number }>>`
    ...
    const quotes = watchlist.items.map((item) => {
      ...
      return {
        ...
        change,
        changePercent: change != null && prevClose ? +(change / prevClose * 100).toFixed(2) : null,
