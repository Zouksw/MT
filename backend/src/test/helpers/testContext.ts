...
export async function createTestContext(suiteName: string): Promise<TestContext> {
  ...
  try {
    prisma = new PrismaClient({ log: ['error'], datasources: { db: { url: dbUrl } } });
...
export async function destroyTestContext(ctx: TestContext): Promise<void> {
  if (!ctx.available) {
    ...
    try { await ctx.redis.quit(); } catch {};
    return;
  }

  // Clean up PostgreSQL data by prefix
  try {
    await ctx.prisma.apiKey.deleteMany({ where: { name: { startsWith: ctx.prefix } } });
  } catch {}
  try {
    await ctx.prisma.watchlist.deleteMany({ where: { name: { startsWith: ctx.prefix } } });
  } catch {}
  try {
    await ctx.prisma.simulationAccount.deleteMany({ where: { name: { startsWith: ctx.prefix } } });
  } catch {}
  try {
    await ctx.prisma.dataset.deleteMany({ where: { slug: { startsWith: ctx.prefix } } });
  } catch {}
  try {
    await ctx.prisma.user.deleteMany({ where: { email: { startsWith: ctx.prefix } } });
  } catch {}
  ...
  try {
    let cursor = 0;
    do {
      const result = await ctx.redis.scan(cursor, { MATCH: `*${ctx.prefix}*`, COUNT: 100 });
