...
export async function setupTestDatabase(): Promise<{ prisma: PrismaClient }> {
  testPrisma = new PrismaClient({
    datasources: {
      db: {
        url: process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/iotdb_enhanced_test',
      },
    },
    log: ['error'],
  });
 ...
 */
export async function cleanupRedis(redis: ReturnType<typeof createClient>, prefix = 'test:') {
  try {
...
export async function createDatabaseDataset(
  ...
  if (!org) {
    org = await prisma.organizations.create({
      data: { id: 'test-org-id', name: 'Test Org', slug: 'test-org', owner_id: userId },
    });
  }
  ...
  const dataset = await prisma.dataset.create({
    data: {
      name,
      slug: overrides.slug || name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, ''),
