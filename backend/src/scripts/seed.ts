...
async function main() {
	...
	for (let i = 0; i < (hours * 60 * 60 * 1000) / interval; i++) {
		...
		// Pressure datapoint
		const pressureValue = parseFloat(
			(1000 + Math.sin(i / 12) * 50 + (Math.random() - 0.5) * 20).toFixed(2),
		);
		datapoints.push({
		...
		// Vibration datapoint
		const vibrationValue = parseFloat(
			(50 + Math.sin(i / 6) * 10 + (Math.random() - 0.5) * 5).toFixed(2),
		);
		datapoints.push({
	...
	for (let i = 0; i < datapoints.length; i += batchSize) {
		...
		console.log(
			`✅ Inserted datapoints batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(datapoints.length / batchSize)}`,
		);
	}
