...
export function createJwtUtilsMock() {
	return {
		...
		verifyToken: vi
			.fn()
			.mockReturnValue({ userId: "test-user-id", exp: Date.now() + 3600000 }),
...
export function createConfigMock() {
	return {
		jwt: {
			secret: "test-secret-key-for-jwt-testing-purposes-only-32chars",
			expiresIn: "1h",
		},
...
export const MockNotFoundError = class extends Error {
	statusCode = 404;
};
export const MockBadRequestError = class extends Error {
	statusCode = 400;
};
export const MockForbiddenError = class extends Error {
	statusCode = 403;
};
export const MockUnauthorizedError = class extends Error {
	statusCode = 401;
};
export const MockConflictError = class extends Error {
	statusCode = 409;
};
