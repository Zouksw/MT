...
if (
	...
	iotdbConfig.username === "root" &&
	iotdbConfig.password === "root"
) {
	logger.warn(
		"SECURITY WARNING: Using default IoTDB credentials in production. Change IOTDB_USERNAME and IOTDB_PASSWORD.",
	);
}

if (iotdbConfig.username === "root" || iotdbConfig.password === "root") {
	logger.warn(
		"IoTDB is using default credentials. Consider using a dedicated user.",
	);
}
