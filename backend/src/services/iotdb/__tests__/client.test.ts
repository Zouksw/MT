...
describe("IoTDBClient", () => {
	...
	describe("query", () => {
		...
		it("should handle query errors", async () => {
			...
			await expect(client.query("SELECT * FROM root.test1")).rejects.toThrow(
				"IoTDB API error",
			);
		});
		...
		it("should handle authentication errors in response", async () => {
			mockFetch.mockResolvedValue(
				mockSuccessResponse({ code: 401, message: "Authentication failed" }),
			);
	...
	describe("createTimeseries", () => {
		it("should create timeseries with valid parameters", async () => {
			mockFetch.mockResolvedValue(
				mockSuccessResponse({ code: 200, message: "Success" }),
			);
	...
	describe("deleteTimeseries", () => {
		it("should delete timeseries successfully", async () => {
			...
			await expect(
				client.deleteTimeseries("root.test1.temp"),
			).resolves.not.toThrow();
		});
	...
	describe("listTimeseries", () => {
		it("should list all timeseries when no path specified", async () => {
			mockFetch.mockResolvedValue(
				mockSuccessResponse({ code: 200, timeseries: [] }),
			);
		...
		it("should list timeseries for specific path", async () => {
			mockFetch.mockResolvedValue(
				mockSuccessResponse({ code: 200, timeseries: ["root.test1.temp"] }),
			);
	...
	describe("Security - Path Validation", () => {
		const maliciousPaths = [
			...
			{
				path: "root.test; INSERT INTO--",
				description: "SQL injection with INSERT",
			},
			{
				path: "root.test' UNION SELECT--",
		...
		it.each(
			maliciousPaths,
		)("should reject malicious path: $description", async ({ path }) => {
			await expect(
				client.createTimeseries({
					path,
					dataType: "DOUBLE",
		...
		const validPaths = [
			...
			{
				path: "root.device_1.temp-sensor",
				description: "with hyphen and underscore",
			},
			{ path: "root.test-db.*", description: "with wildcard" },
			{ path: "root.device_1.sensor_2", description: "all valid characters" },
		];

		it.each(validPaths)("should accept valid path: $description", async ({
			path,
		}) => {
			mockFetch.mockResolvedValue(mockSuccessResponse({ code: 200 }));
	...
	describe("Security - Type Validation", () => {
		...
		it.each(
			validDataTypes,
		)("should accept valid data type: %s", async (dataType) => {
			mockFetch.mockResolvedValue(mockSuccessResponse({ code: 200 }));
		...
		it.each(
			validEncodings,
		)("should accept valid encoding: %s", async (encoding) => {
			mockFetch.mockResolvedValue(mockSuccessResponse({ code: 200 }));
