...
const DATE_PATTERNS = ['date', 'time', 'timestamp', 'datetime', 'trade_date', 'trading_date', 'day', 'period'];
...
const CLOSE_PATTERNS = ['close', 'close_price', 'closing', 'closeprice', 'last', 'settle', 'settlement', 'price', 'value', 'end_price'];
...
function matchHeader(headers: string[], patterns: string[]): string | undefined {
  const normalized = headers.map(h => h.toLowerCase().replace(/[\s_-]+/g, '_').trim());
...
export function normalizePriceEntry(
  ...
  return {
    ...
    source: (mapping.source && raw[mapping.source] ? String(raw[mapping.source]) : 'manual') as string,
    metadata: Object.keys(metadata).length > 0 ? metadata : undefined,
  };
}
