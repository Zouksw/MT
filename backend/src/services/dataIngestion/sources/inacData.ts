...
const INAC_EXPORT_URL =
	"https://www.inac.gub.uy/estadisticas/exportaciones.html";
	...
	try {
		...
		if (res.ok) {
			...
			for (const match of tableMatches) {
				...
				const cellValues = Array.from(cells).map((c) =>
					c[1].replace(/<[^>]+>/g, "").trim(),
				);
		...
		logger.warn(
			`[INAC] Live fetch failed: ${err instanceof Error ? err.message : err}`,
		);
	}
