...
async function fetchBalticDry(): Promise<ScraperResult> {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json() as { date: string; value: number; change: number; cape: number; panamax: number; supramax: number };
...
async function upsertBDI(
  date: Date,
  value: number,
  extra: { cape?: number; panamax?: number; supramax?: number; change?: number; source: string },
): Promise<{ inserted: number; updated: number }> {
  let inserted = 0;
  let updated = 0;

  let commodity = await prisma.commodity.findUnique({ where: { slug: BDI_SLUG } });
  if (!commodity) {
  ...
  const existing = await prisma.commodityPrice.findUnique({
    where: { commodityId_interval_date_source: { commodityId: commodity.id, interval: 'daily', date, source: 'baltic' } },
  });
