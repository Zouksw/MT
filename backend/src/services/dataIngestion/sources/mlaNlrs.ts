...
async function fetchMLAData(): Promise<ScraperResult> {
  ...
  if (othPrices.length > 0 || exportPrices.length > 0) {
        ...
        try {
          await prisma.beefCutPrice.upsert({
            ...
            create: {
              ...
              source: 'mla_oth',
              date,
              metadata: { audPerKg, category: item.category, factoryCode: item.factoryCode } as unknown as Prisma.InputJsonValue,
            },
          });
          inserted++;
        } catch {
    ...
    const auFactory = await prisma.factory.findFirst({ where: { country: 'AU' } });
      ...
      try {
        await prisma.beefCutPrice.upsert({
          ...
          create: {
            ...
            source: 'mla_export',
            date,
            metadata: { rawName: item.cutName } as unknown as Prisma.InputJsonValue,
          },
        });
        inserted++;
      } catch {
