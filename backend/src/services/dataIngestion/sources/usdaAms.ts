...
async function fetchCutLevelPrices(): Promise<ScraperResult> {
  ...
  const usFactory = await prisma.factory.findFirst({ where: { country: 'US' } });
  if (!usFactory) {
    ...
    try {
      await prisma.beefCutPrice.upsert({
        ...
        create: {
          ...
          metadata: { rawName: cutName, originalUnit: 'USD/cwt', originalPrice: price } as unknown as Prisma.InputJsonValue,
        },
      });
      inserted++;
    } catch {
...
async function generateCutEstimates(): Promise<ScraperResult> {
  ...
  const basePrices: Record<string, number> = {
    RIB_EYE_ROLL: 18.5, STRIPLOIN: 15.8, TENDERLOIN: 32.0, BRISKET_NAVEL: 8.2,
    CHUCK_ROLL: 9.5, TOPSIDE: 7.0, SILVERSIDE: 6.5, OUTSIDE_SKIRT: 14.0,
    INSIDE_SKIRT: 13.5, SHORT_RIBS: 15.0, KNUCKLE: 7.5, EYE_ROUND: 6.0,
    BLADE: 10.5, FLAP: 8.5, TRI_TIP: 12.0, SIRLOIN: 13.0,
    RIB_CAP: 22.0, HANGING_TENDER: 11.0, FORESHANK: 5.5,
  };
