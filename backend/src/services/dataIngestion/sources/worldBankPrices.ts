...
const WB_COMMODITIES: Record<string, { slug: string; name: string; unit: string }> = {
  // Energy
  CRUDE_WTI:    { slug: 'crude_oil_wti',      name: 'Crude Oil (WTI)',       unit: 'USD/bbl' },
  CRUDE_BRENT:  { slug: 'crude_oil_brent',     name: 'Crude Oil (Brent)',     unit: 'USD/bbl' },
  CRUDE_DUBAI:  { slug: 'crude_oil_dubai',     name: 'Crude Oil (Dubai)',     unit: 'USD/bbl' },
  NATURAL_GAS:  { slug: 'natural_gas_us',       name: 'Natural Gas (US)',      unit: 'USD/MMBtu' },
  COAL_AUS:     { slug: 'coal_australia',        name: 'Coal (Australia)',      unit: 'USD/ton' },

  // Metals
  COPPER:       { slug: 'copper_lme',           name: 'Copper (LME)',          unit: 'USD/ton' },
  ALUMINUM:     { slug: 'aluminum_lme',         name: 'Aluminum (LME)',        unit: 'USD/ton' },
  IRON_ORE:     { slug: 'iron_ore_cfr',         name: 'Iron Ore (CFR China)',  unit: 'USD/ton' },
  ...
  PLATINUM:     { slug: 'platinum_lbma',        name: 'Platinum (LBMA)',       unit: 'USD/troy oz' },

  // Grains & Oilseeds
  SOYBEANS:     { slug: 'soybeans_cbOT',        name: 'Soybeans (CBOT)',       unit: 'USD/ton' },
  SOYBEAN_MEAL: { slug: 'soybean_meal_cbOT',    name: 'Soybean Meal (CBOT)',   unit: 'USD/ton' },
  SOYBEAN_OIL:  { slug: 'soybean_oil_cbOT',     name: 'Soybean Oil (CBOT)',    unit: 'USD/ton' },
  ...
  COFFEE_ARAB:  { slug: 'coffee_arabica',       name: 'Coffee (Arabica)',      unit: 'USD/kg' },
  COFFEE_ROB:   { slug: 'coffee_robusta',       name: 'Coffee (Robusta)',      unit: 'USD/kg' },
  COCOA:        { slug: 'cocoa',                name: 'Cocoa',                 unit: 'USD/kg' },
  TEA:          { slug: 'tea_mombasa',           name: 'Tea (Mombasa)',         unit: 'USD/kg' },
  COTTON:       { slug: 'cotton_cotlook',        name: 'Cotton (Cotlook A)',    unit: 'USD/kg' },
  ...
  LAMB:         { slug: 'lamb_new_zealand',       name: 'Lamb (New Zealand)',    unit: 'USD/kg' },
  SHRIMP:       { slug: 'shrimp_latam',           name: 'Shrimp (Latin America)', unit: 'USD/kg' },
  ...
  DAP:          { slug: 'diammonium_phosphate',  name: 'DAP Fertilizer',        unit: 'USD/ton' },
  POTASH:       { slug: 'potassium_chloride',    name: 'Potash (MOP)',          unit: 'USD/ton' },
  TSP:          { slug: 'triple_superphosphate', name: 'TSP Fertilizer',        unit: 'USD/ton' },

  // Indices
  COMMODITY_INDEX: { slug: 'commodity_price_index', name: 'Commodity Price Index', unit: 'index' },
};
...
async function fetchWorldBankData(): Promise<ScraperResult> {
  ...
  const url = 'https://api.worldbank.org/v2/commodity?format=json&per_page=5000&date=2020:2026';
  ...
  for (const [code, config] of Object.entries(WB_COMMODITIES)) {
    const matchingRecords = [...grouped.entries()]
      .flatMap(([recCode, recs]) =>
    ...
    for (const record of recentRecords) {
      ...
      if (existing) {
        ...
        await prisma.commodityPrice.create({
          data: { commodityId: commodity.id, date, interval: 'monthly', ...priceData },
        });
        inserted++;
      }
    }
...
function categorizeSlug(slug: string): string {
  if (slug.includes('crude') || slug.includes('natural_gas') || slug.includes('coal')) return 'energy';
  if (slug.includes('copper') || slug.includes('aluminum') || slug.includes('iron') || slug.includes('lme') || slug.includes('gold') || slug.includes('silver') || slug.includes('platinum') || slug.includes('nickel') || slug.includes('zinc') || slug.includes('lead') || slug.includes('tin')) return 'metals';
  if (slug.includes('urea') || slug.includes('phosphate') || slug.includes('potassium') || slug.includes('potash') || slug.includes('tsp')) return 'fertilizer';
  if (slug.includes('beef') || slug.includes('chicken') || slug.includes('lamb') || slug.includes('shrimp') || slug.includes('butter') || slug.includes('milk')) return 'meat_dairy';
  if (slug.includes('sugar') || slug.includes('coffee') || slug.includes('cocoa') || slug.includes('tea') || slug.includes('cotton') || slug.includes('rubber')) return 'soft_commodities';
  if (slug.includes('soybean') || slug.includes('wheat') || slug.includes('corn') || slug.includes('rice') || slug.includes('barley') || slug.includes('sorghum')) return 'grain';
