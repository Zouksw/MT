    ...
    if (res.ok) {
      ...
      if (priceMatch) {
        ...
        if (!Number.isNaN(brlPerArroba)) {
          ...
          let commodity = await prisma.commodity.findUnique({ where: { slug: 'boi_gordo_br' } });
          if (!commodity) {
          ...
          await prisma.commodityPrice.upsert({
            ...
            create: {
              ...
              close: brlPerArroba,
              source: 'cepea',
              metadata: { brlPerArroba, usdPerKg: parseFloat(usdPerKg.toFixed(2)) } as unknown as Prisma.InputJsonValue,
            },
          });
          inserted++;
        }
