...
const FAO_INDICES: Record<string, { itemCode: string; elementCode: string; slug: string }> = {
  food_index: { itemCode: '21012', elementCode: '5510', slug: 'fao_food_index' },
  meat_index: { itemCode: '21013', elementCode: '5510', slug: 'fao_meat_index' },
  dairy_index: { itemCode: '21014', elementCode: '5510', slug: 'fao_dairy_index' },
  cereals_index: { itemCode: '21015', elementCode: '5510', slug: 'fao_cereals_index' },
  oils_index: { itemCode: '21017', elementCode: '5510', slug: 'fao_oils_index' },
};
...
async function fetchFAOIndex(itemCode: string, elementCode: string): Promise<FAOResponse['data']> {
  ...
  try {
    ...
    logger.warn(`[FAO] Fetch ${itemCode} failed: ${err instanceof Error ? err.message : err}`);
    return [];
  }
}
...
async function updateFAOPrices(): Promise<ScraperResult> {
  ...
  for (const [, config] of Object.entries(FAO_INDICES)) {
    ...
    const priceData = {
      ...
      volume: null,
      source: 'fao_faostat',
      metadata: { itemCode: config.itemCode, year, flag: latest.Flag } as unknown as Prisma.InputJsonValue,
    };
