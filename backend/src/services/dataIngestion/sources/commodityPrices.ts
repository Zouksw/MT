...
function getSeasonalFactor(month: number): number {
	...
	// Low in summer
	const factors = [
		0.06, 0.08, 0.02, -0.02, -0.04, -0.05, -0.04, -0.02, 0.02, 0.06, 0.04, 0.03,
	];
...
async function updateCommodityPrices(): Promise<ScraperResult> {
	...
	for (const commodity of commodities) {
		...
		const open = parseFloat(
			(basePrice + (Math.random() - 0.5) * model.volatility * 0.3).toFixed(4),
		);
		const high = parseFloat(
			Math.max(
				open,
				close,
				close + Math.random() * model.volatility * 0.5,
			).toFixed(4),
		);
		...
		if (
			commodity.category === "beef_cuts" &&
			commodity.originCountry !== "CN"
		) {
			metadata = {
				spot_cny_kg: close,
