...
const DCE_PRODUCTS: Record<string, {
  slug: string;
  name: string;
  nameCn: string;
  unit: string;
  ...
  m: { slug: 'soybean_meal_dce', name: 'Soybean Meal (DCE)', nameCn: '豆粕（大商所）', unit: 'CNY/ton', basePrice: 3200 },
  c: { slug: 'corn_dce', name: 'Corn (DCE)', nameCn: '玉米（大商所）', unit: 'CNY/ton', basePrice: 2450 },
  p: { slug: 'palm_oil_dce', name: 'Palm Oil (DCE)', nameCn: '棕榈油（大商所）', unit: 'CNY/ton', basePrice: 8200 },
  i: { slug: 'iron_ore_dce', name: 'Iron Ore (DCE)', nameCn: '铁矿石（大商所）', unit: 'CNY/ton', basePrice: 780 },
  j: { slug: 'coke_dce', name: 'Coke (DCE)', nameCn: '焦炭（大商所）', unit: 'CNY/ton', basePrice: 2200 },
  y: { slug: 'soybean_oil_dce', name: 'Soybean Oil (DCE)', nameCn: '豆油（大商所）', unit: 'CNY/ton', basePrice: 7800 },
};

const CZCE_PRODUCTS: Record<string, {
  slug: string;
  name: string;
  nameCn: string;
  unit: string;
  ...
  CF: { slug: 'cotton_czce', name: 'Cotton (CZCE)', nameCn: '棉花（郑商所）', unit: 'CNY/ton', basePrice: 14800 },
  SR: { slug: 'sugar_czce', name: 'Sugar (CZCE)', nameCn: '白糖（郑商所）', unit: 'CNY/ton', basePrice: 6200 },
  WH: { slug: 'wheat_czce', name: 'Strong Wheat (CZCE)', nameCn: '强麦（郑商所）', unit: 'CNY/ton', basePrice: 3100 },
  TA: { slug: 'pta_czce', name: 'PTA (CZCE)', nameCn: 'PTA（郑商所）', unit: 'CNY/ton', basePrice: 5600 },
};
...
async function fetchDCEFutures(): Promise<ScraperResult> {
  ...
  for (const [symbol, config] of Object.entries(DCE_PRODUCTS)) {
    try {
      ...
      if (!res.ok) continue;
      const data = await res.json() as { data?: Array<{ settlement: string; date: string; open: string; high: string; low: string; volume: string }> };
  ...
  for (const [symbol, config] of Object.entries(CZCE_PRODUCTS)) {
    try {
      ...
      if (!res.ok) continue;
      const data = await res.json() as { data?: Array<{ settlement: string; date: string; open: string; high: string; low: string }> };
      if (!data.data?.length) continue;
...
async function upsertFuturesPrice(
  config: { slug: string; name: string; nameCn: string; unit: string; basePrice: number },
  settle: number,
  ...
  const existing = await prisma.commodityPrice.findUnique({
    where: { commodityId_interval_date_source: { commodityId: commodity.id, interval: 'daily', date, source: exchange } },
  });
  ...
  const priceData = {
    ...
    volume,
    source: exchange,
    metadata: { exchange, rawDate: dateStr } as unknown as Prisma.InputJsonValue,
  };
