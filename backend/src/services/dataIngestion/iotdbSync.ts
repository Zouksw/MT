 ...
 */
export async function syncCommoditiesToIoTDB(): Promise<{
	synced: number;
	skipped: number;
}> {
	let synced = 0;
 ...
 */
export async function getCommodityTimeseriesPath(
	slug: string,
): Promise<string | null> {
...
export async function scheduleAllCommodityPredictions(): Promise<number> {
	const { scheduleRecurringPredictions } = await import(
		"@/services/predictionQueue"
	);
	...
	for (const commodity of commodities) {
		...
		try {
			...
			logger.warn(
				`[Predictions] Failed to schedule for ${commodity.slug}: ${err}`,
			);
		}
	}

	logger.info(
		`[Predictions] Scheduled recurring predictions for ${scheduled}/${commodities.length} commodities`,
	);
	return scheduled;
}
