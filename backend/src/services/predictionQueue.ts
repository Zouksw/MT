...
export async function initPredictionQueue(): Promise<void> {
  ...
  predictionWorker = new Worker<PredictionJobData>(
    ...
    async (job: Job<PredictionJobData>) => {
      ...
      logger.info(`Processing prediction job for ${commodityId} (${models.length} models)`);
      ...
      logger.info(`Prediction job complete: ${succeeded}/${models.length} models succeeded`);
    },
  ...
  correlationWorker = new Worker<CorrelationJobData>(
    ...
    async (job: Job<CorrelationJobData>) => {
      ...
      logger.info(`Computing correlation matrix for ${commodityIds.length} commodities`);
...
export async function scheduleRecurringPredictions(
  ...
  logger.info(`Scheduled recurring predictions for ${commodityId} (every 30 min)`);
}
