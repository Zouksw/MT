...
import { createAlertRule, evaluateAlertRule } from "@/services/alert-rules";
...
describe("Alert Rules Complex Integration Tests", () => {
	...
	describe("Threshold Condition Edge Cases", () => {
		...
		test("should handle = operator with precision", async () => {
			...
			expect(await evaluateAlertRule(rule, createEvalData(100.0000001))).toBe(
				false,
			);
			expect(await evaluateAlertRule(rule, createEvalData(99.9999999))).toBe(
				false,
			);
		});
		...
		test("should handle special numeric values", async () => {
			...
			expect(await evaluateAlertRule(rule, createEvalData(Infinity))).toBe(
				true,
			);
			expect(await evaluateAlertRule(rule, createEvalData(-Infinity))).toBe(
				false,
		...
		test("should handle very large and very small numbers", async () => {
			...
			expect(
				await evaluateAlertRule(rule, createEvalData(Number.MIN_VALUE)),
			).toBe(true);

			// Very large negative number (should not trigger)
			expect(
				await evaluateAlertRule(rule, createEvalData(-Number.MAX_VALUE)),
			).toBe(false);
		});
	});
	...
	describe("Pattern Condition (Placeholder)", () => {
		test("should handle pattern condition (placeholder)", async () => {
			...
			expect(await evaluateAlertRule(rule, createEvalData(100, {}))).toBe(
				false,
			);
		});
	});
	...
	describe("Concurrent Evaluations", () => {
		...
		test("should handle multiple rules concurrently", async () => {
			...
			const results = await Promise.all(
				rules.map((rule) => evaluateAlertRule(rule, data)),
			);
