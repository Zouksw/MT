...
**round-171 执行记录（2026-09-20，外部信息扩容轮——三新源 + 四死路登记）**：应"引入更大量外部相关信息"目标，先按 §十.1 完善度评估锁定信息缺口（全球月度基准缺指数层、供需/展望层为 0、贸易镜像无 UK），后 live 探测落地三源：①**`fao_index`**——FAO 六指数族月度（Food/Meat/Dairy/Cereals/Oils/Sugar，2014-16=100）1990-01→t-1 共 **2640 行**入 CommodityPrice 月度，fao_meat_index 自动过月度预测门（90 天窗+≥3 点），实测 on-demand chronos 3 步月度预测通；增量 3 月尾窗+noChange；牛因子子指数不在免费文件，不造。②**`oecd_outlook`**——OECD-FAO 展望牛因子供需平衡（QP/QC/IM/EX × 8 区域 × 1990→2035，千吨口径实测校准 CHN QP 2024=7.79Mt/BRA EX=3.78Mt）**1467 行**入 MarketFactor，**2027-2035 九个预测年 projection=true 如实标注**——平台首个"官方预测"数据类别；237MB/773,719 行流式解析（Response.body+quote-aware 切分），服务端过滤已死（commodity 404/area 忽略）；7 天门读自身 success 日志；PP 本币价/WP 零散世界价量纲不可靠不落；URY 未被建模（走 inac）。③**`hmrc_ots`**——UK OTS OData（OGL 3.0）GB→CN 月度牛肉 **GBP/ton 派生单价**入镜像家族（export_uk_to_cn_0201/0202）；UK 脱欧后唯一月度程序化通道（Comtrade 无伙伴明细实测 0 行）；量级小（£4k-188k/月）为覆盖性通道；抑制月诚实跳过。**工程教训**：HMRC WAF 拦 OData `in (...)` 谓词（in-1 即 403、or 链 200，报文级二分）——与 jwqyp 的 Accept-Language 500 同类"隐式契约"问题。**死路登记（不再试）**：Comtrade 伙伴国扩容（PY 仅 World 总额、EU 成员仅年度且 Comext 月度已覆盖、GB 无伙伴拆分、预览 API 单次 1 period）；RSS 扩源（meatpoultry topic 停更 2018/2026-05、beefcentral 外全灭：beefinternational 301→000/mla 404/fas 403/chordata 000/foodmate 无 RSS——当前双 feed 即免费面全部）；FBX 维持登记。门禁：backend **1207+1 skip（117 文件，+21）**零回退（fao 首跑 1 例失败 4 连复跑全绿，登记不掩盖）；live 实证三源首灌+复触 noChange（fao 670ms/oecd 门 4.1s/hmrc 348ms）。数据源注册 26→**29**（文件 28→31）。
