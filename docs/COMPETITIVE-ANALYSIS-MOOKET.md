...
3. **AI 叙事与实测倒挂**：营销主打 3 Chronos 共识，但生产库 148,245 条预测的验证集实测（2026-08-23）：
   ...
   双口径合起来的准确结论：**典型序列上 chronos 不差（中位 <1%），但它有无界灾难长尾**（均值被劣化序列拉高 10 倍以上，naive 的最坏情况有界）。风险视角下均值才是对的度量——采购决策毁于爆仓日而非中位数日。被营销领衔的模型恰是尾部风险最大的模型，且共识集成未按序列路由/加权，整体继承该尾部风险。（中位数普遍 <1% 同时提示验证集多为易预测序列——评估口径本身待复核，另见 KNOWN-ISSUES MAPE 验证环。）
...
1. **可验证的预测机制本身**：9 模型 + 自动 MAPE 验证 + accuracy 对比页——牧集公开面无预测产品。要拥抱实测结论（短月度序列上统计基线胜过预训练基础模型，与文献一致），把卖点从"大模型"换成"**可验证**"。
...
- 共识机制按实测 MAPE 加权或按序列长度路由模型（chronos 限长序列 / 短序列交给统计基线），消除尾部风险。
