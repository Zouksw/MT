...
### A1. 数据源健康审计与激活 — **第 1 轮**
- 逐个激活/修复 20 个数据源,记录每个源的:能否拉到数据、最新数据日期、拉取频率
- **验证标准**: 启动一次完整 scrape 后,记录每源 `inserted/updated` 计数
- **已知现状**: ROADMAP 旧版标注 FRED/OPENWEATHER/USDA_MARS key 为 Pending,
  实测 `.env` 中 4 个 key 已配置 (FRED/OPENWEATHER/MLA/USDA_MARS),需验证是否真的产生数据
- **已知待修**: World Bank 源 (API 404 → HTML scrape,旧 ROADMAP 标注)
...
### 第 1 轮 — 地基 (基础先行)
- **A**: 数据源健康审计,激活 4 个已配 key 的源,实测产出
- **B**: B1 Simulation 重定位的产品方案确认 + 数据迁移设计
- **C**: **C1 进程内测试重构** (解锁后续所有验证)
- **质量门**: tsc / build / 全量测试无 429 假失败
...
|------|------|------|
| 商品数据覆盖率 | 34.5% | **≥60%** |
| 依赖漏洞 (high+critical) | 51 | **0** |
| 胖路由 (>600 行) | 6 | **0** |
| 路由直连 Prisma | 208 处 | **<30** (仅简单 CRUD) |
| 测试 429 假失败 | 存在 | **0** (进程内测试) |
| 前端数据获取模式 | 2 套 | **1 套 (SWR)** |
| Simulation 定位 | 模拟交易 | 回测工具 |
