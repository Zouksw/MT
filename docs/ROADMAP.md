...
| 指标 | 起点 | 当前(Round 27 探查后, 2026-07-12 实测) | 目标 |
|------|------|------|------|
| **prediction_logs 落库** | 1066 全 pending | **36,810 completed + 90 verified** ✅(Round 12 状态机 + Round 18 吞错 + Round 27 MAPE 解堵)| **持续增长 + verified 率 >50%** |
| ~~forecasts 表~~ | ~~0~~ | — | **指标废弃**(架构错配:预测存 Redis 缓存,forecasts 仅训练路径写;改用 prediction_logs) |
| **MAPE 可验证率** | — | **90/36,810 (0.24%)** ⚠️ | **>50%** (依赖主线 A 数据流入 + Round 28 brl_usd 96% 异常排查) |
| **MAPE 均值(质量)** | — | FX 0.14-1.25% ✅ / brl_usd 96% ⚠️ | **<30%** |
| **有数据商品** | 34.5% | **40/110 (36.4%)** ⬆ | **≥60%** |
| **市场因子类型** | 131(声称)| **1 type / 48 行** | **多类型覆盖** |
| 商品数据覆盖率 | 34.5% | **36.4%** ⬆ | **≥60%** |
| 依赖漏洞 (high+critical) | 51 | **后端 16 (1C/5H) + 前端 18 (6H) = 34** ⬇ | **0** |
| 胖路由 (>600 行) | 6 | **0** ✅ | **0** |
| 路由直连 Prisma | 208 处 | **~57** (watchlist 17/beef 15/portfolios 15/timeseries 10 — 4 路由无 service)| **<30** |
| 测试 429 假失败 | 存在 | **0** ✅ | **0** |
| 前端测试可运行 | ❌ | **✅**(272/272) | ✅ |
| 后端测试 | — | **452 pass / 1 skip** ⬆(+5 Round 26 inference)| ✅ |
| **前端 next build** | ❌(Round 17 前 exit 1) | **✅ exit 0**(Round 17/25) | ✅ |
| inference 服务可达 | ❌ DOWN | **✅ UP**(Round 7/26 slug 修复) | ✅ |
| 磁盘损坏复发 | 3 次 | **✅ 根治**(Round 11) | 0 次 |
| L1 token 化 | 27 处 | **0** ✅ | **0** |
| 覆盖率门槛 | — | **50%** ⚠️(太低,大模块 0% 仍过)| **65-80%** (routes/services 分域) |

> **Round 27 多技能探查(2026-07-12)**: 4 路 Explore agent 并行扫后端架构/前端数据层/数据管线/测试盲区 + review skill 审 PR #1 + ops-check 实测。
> 关键结论: 水管已通(36,810 预测在产)但覆盖率几乎没动(34.5→36.4),瓶颈是数据源(4 源休眠因缺 key)。
> 后续 6 轮(R27-32)计划详见 `reviews/2026-07-12-round-27-exploration.md`。
