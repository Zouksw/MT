...
- **O3（`de82d6b`）**：推理服务收敛 **127.0.0.1**——ecosystem mt-inference uvicorn `--host` 与 `env_production.INFERENCE_HOST` 由 0.0.0.0 改 127.0.0.1（唯一调用方 backend config.ts:65 默认 localhost，仓库内无 frontend 引用 10810），`pm2 delete + start --env production + pm2 save` 三步固化（不 save 则机器重启 resurrect /root/.pm2/dump.pm2 回退 0.0.0.0）。live 验收：ss 仅 127.0.0.1:10810、外网网卡 172.31.45.180:10810 连接失败（私网接口地址——证明本机非回环接口不再监听，公网可达性由云安全组决定，见 KNOWN-ISSUES S3）、/health 200、/health/ready inference alive:true、dump args 含 `--host 127.0.0.1`。
...
- **同日报告修订勘误与补验（live，只读）**：landing 断言补跑通过——'5 markets' 与 'Upgrade for AI forecasts' 均 0 命中、'AI forecasts are open to every registered account.' 1 命中、`tabular-nums">0<` 0 处、75/9/23/6 各 2 处（行动项原文 from-6 正则因不允许 "from" 后空格误报 0，容忍正则 `from(<!-- -->| )+6(<!-- -->| )+markets` 1 命中）；CNY 现货 psql 复测 **22 部位 38 行**（审计快照「19 部位 33 行 / 13×2+6×1」内部不自洽，序列日增点数为移动目标）；牛肉预测精确计数：completed 70（≈0.15%）/ verified 14，「0 条」口径为**部位级**序列（aus_* 三序列全 unverifiable、beef_90cl_us 0 行）；weekly-kill 空数组定案为默认 12 周窗过滤（beef.ts:154-162，`?weeks=52` 实测可取回 190 行中最新者）**非 bug**；`/api/market/sources` 无 token 401（鉴权门内，count=29 未经凭证核认）；`/root/.secrets.tmp` 实测不存在（SECRETS-MANAGEMENT.md:20 提及的历史文件）；KNOWN-ISSUES 同步勘误（D1 数字、D6 根因、S1 定级前提）并新增 **S9**（seed 供给卡已定位 beef/page.tsx:246-250 + 根路径 JS 跳板源码证实等四项审计遗留追踪）。
...
- **厂号挂牌归属**：标题**前导厂号**提取（`^(\d{2,5})(?=[^0-9A-Za-z箱件吨柜])`——"2543牛腩肋条"→2543、"30和牛碎肉65"→30；中段数字（谷饲4302）与 VL/箱件吨量词排除——错归属随 day-key 首见固化不可逆，宁缺勿错）+ 产地中文→ISO2 精确映射（23 国）+ `{ISO2}-{厂号}` 工厂码（BR 沿用 seed `BR-SIF` 前缀惯例）；CN 产地/未知产地不归属（国产挂价聚合在 RJS-SPOT 语义正确，厂号仍存 metadata）；同码异国冲突跳过归属仅告警。工厂 metadata 标 `gacc-plant-unverified`——厂号读自挂牌标题，身份未经 GACC/MAPA 名册核验（P1 名册批待落地）。
- **day-key 语义升级**：(factory, cut, day) 首见胜出——同日不同厂同部位**双双落行**，现货序列从"平台聚合"升为"每厂粒度"。
- **当日数据回迁**（备份 `backups/round168/`）：BR-SIF2543←2543牛腩肋条 RIB_FINGER 86.5 CNY、NZ-30←30和牛碎肉65 LEAN_TRIM_80 50.0 CNY；/api/beef/factories live 可见两新厂；其余 9 行（含中段 4302 诚实漏过）留 RJS-SPOT。
- **登记**：成交价采集停摆（详见 RESEARCH P1b 注记 + DATA-SOURCES-EVALUATION round-168 记录）；中段厂号（谷饲4302 型）与词表未映射标题（牛杂/牛脂肪/带皮牛头）同为观察窗后的扩词批候选。
- **基线**：backend **1175+1 skip**（+8 全为新测试：提取器 5 + 工厂码 1 + 归属/回退/冲突集成 3）；frontend 365 不变（未触碰）；tsc 0 / biome 0 / build + PM2 重启 + /health 200 + 工厂端点 live 验证。
...
- **批C 肉交所现货源勘察与设计登记（docs-only，实现留独立轮）**：live 复核 `/sell/` 列表契约（40 项/页、`index-htm-page-N` 翻页至 ~16,979 页、元/公斤标记 20 处/页、全品类混排、robots 禁 `/*search*` 故牛肉过滤须客户端做）+ 详情页字段（价格/数量/产地/仓库位置/品类面包屑）实测；两阶段落库设计（一阶段无厂号挂价 market 行 + CNY 币种注记，二阶段厂号成交价待厂号映射批）+ 单位混用与挂价≠成交价诚实边界，登记于 RESEARCH-BEEF-TRADE-DATA-SOURCES.md P1b。
