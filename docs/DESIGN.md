...
| 2026-09-07 | Language rule, zh-CN chrome (round-160 批B) | Audience = Chinese beef-trade professionals. Product chrome (page titles, descriptions, card/section headers, table headers, KPI labels, breadcrumbs, buttons, empty states) is zh-CN; data values, model names (Chronos-T5-Tiny, ARIMA), proper nouns (USDA, MAPE, M9), units and source ids stay English. Auth (/login, /register) and marketing pages (/landing, /about, /pricing) remain English as a deliberate brand choice. |
| 2026-09-07 | Dead-column rule (round-160 批A) | A table column that is 100% "—"/0 reads as a broken pipeline, not honesty: hide it conditionally (dashboard 7 天预测, beef hub 7d Forecast) or replace it with the honest reason (digest 较上期 → 首期，暂无对比). Applied on top of the round-155-era rule for market-news Views. |
| 2026-09-07 | Empty-state compactness (round-160 批C) | An empty state must reserve only the height it needs: a quiet 250px chart box collapsed to a one-line check + explanation + action (dashboard 告警分布); pages whose only function returns nothing render a centered reason instead of a dead grid (analysis 相关矩阵). |
