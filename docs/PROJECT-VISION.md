...
> ## ⚠️ 2026-08-30 状态指针（最新）
>
> 08-23 之后的轮次（125-138）已把第二、三波计划全部执行完毕：
> - **v2.0.0 第二波全落地**（round-130/131）：月度验证生命周期（ADR-0001 Accepted：步长语义/新点门控/节奏感知清扫）+ /trading 牛肉模式可用 + 公信力口径统一 + 数据新鲜度可观测 + CSV 验证一键化。
> - **v3.1.0 第三波七批全落地**（round-135-138）：月度误杀修复与 horizon [1,3]、牛肉月度滚动回测（36 origins；结论：可预测上限在 naive 附近、arima 冷启动冠军、chronos 原生区间欠覆盖 58-78%）、per-series 冠军路由（≥20 verified 激活，beef 预计 2026-11）、方向准确率指标（live chronos 68-70%）、牛肉预测中心 `/beef/forecast` + dashboard hero 同源共识、领先指标双臂门禁（两臂 FAIL 诚实关闭）、FRED 蛋白族 3 序列（牛肉月度 1→2 + 猪/鸡替代蛋白 2）。
> - **当前基线（2026-08-30 round-140 门禁记录）**：backend **1029 pass + 1 skip**（99 文件）/ frontend **338**（38 套件）/ inference **61** 全绿（v3.2.0 收官轮删功能的 −13/−5 已注明）；三服务 PM2 在线；前端 47 页；Prisma 25 模型 / 后端 17 路由（round-140 同步 AGENTS.md）。
> - **下一里程碑是有机事件（零开发）**：牛肉月度 H=1 首批 2026-08-31 到期（实际值等 FRED 8 月点）→ **2026-09 中下旬首批 beef verified 进榜**。v3.2.0 第四波已全部落地（快照 cron / 18 slugs 多源声明 / 进口成本计算器 / D1-D4 卫生处置）；**后续规划见 `docs/IMPROVEMENT-PLAN.md` v3.3.0 第五波**（分发期：公开中文摘要面 + SEO 基建 / seed 身份对齐 / 正确性收尾；批 0a 值守与批 0b 校准区间承接）。
> - 当前事实入口：`AGENTS.md` §三 + `docs/IMPROVEMENT-PLAN.md`（v3.2.0）+ `docs/KNOWN-ISSUES.md` + `docs/TECH-DEBT.md`。
