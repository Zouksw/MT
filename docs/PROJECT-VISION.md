...
> - **当前基线（2026-08-30 实测）**：backend **1022 pass + 1 skip**（98 文件）/ frontend **334**（37 套件）/ inference **66** 全绿；三服务 PM2 在线；前端 46 页。
> - **下一里程碑是有机事件（零开发）**：牛肉月度 H=1 首批 2026-08-31 到期 → 9 月中实际值发布 → **2026-09 中下旬首批 beef verified 进榜**。后续规划见 `docs/IMPROVEMENT-PLAN.md` **v3.2.0 第四波**（快照自动化 / 多源声明解锁 / 进口成本计算器 / 验证窗值守与校准区间）。
