...
|---|---|---|---|---|
| backend | vitest 4（round-90 从 3 升级） | vitest.config.ts | 94（2026-08-22） | **974 pass / 1 skip** |
...
> 三者合计 **1352 全绿**（974 + 314 + 64，截至 2026-08-22 round-119 实测）。测试数随时间变化，运行 `cd backend && pnpm test`、`cd frontend && pnpm test`、`cd inference-service && pytest -q` 获取当前数。
