...
|---|---|---|---|---|
| backend | vitest 4（round-90 从 3 升级） | vitest.config.ts | 102（2026-08-31） | **1072 pass / 1 skip** |
| frontend | jest 29 + Testing Library | jest.config.js | 40（2026-08-31） | **352 pass** |
| inference | pytest 8 | conftest.py | 4（2026-08-31） | **61 pass** |
| frontend E2E | Playwright | playwright.config.ts | 9 specs | chromium only |

> 三者合计 **1485 全绿**（1072 + 352 + 61，截至 2026-08-31 round-150 实测〔V7 批 1+2+3：backend +18（NW_LS421 解析 10 + noChange 分类器 1 + spec 纯函数 5 + 导入集成 2，第 102 套件）、frontend +3（/trading cadence 选择器）〕；round-146 轨迹：1041+1/341/61〔round-144 基线〕→ 批 1 +6/+4（/api/search 6 + GlobalSearch 组件 4，jest 第 40 套件）→ 批 2 +5（factoryCode 钉住门控/钉住语义/未知厂 404/region×2）→ 批 3 +2（channels-status）= 1054+1/345/61；round-147（/trading ?slug= 深链）frontend +4〔hook 初始选中+未知回退、页面透传/同路由同步/无参不触发、GlobalSearch href〕= 1054+1/349/61；已知首跑 flake 机制同前〔--force 重建后 Redis 预测缓存冷启动，round-136 首例〕；round-122 批 2 起后端测试强制 Redis db1 与生产 db0 隔离；**round-147 新登记第二类环境性失败**：`correlationAnalysis.test.ts` 是唯一对 live 后端（:8000）登录的 HTTP 集成套件，而 `authRateLimiter` 为 **10 次/15 分钟/IP 且成功请求也计数**（`rateLimiter.ts` skipSuccessfulRequests=false）——会话内手动生产登录 + 连续快速全量跑套件会耗尽限流桶，beforeAll 登录 429（`admin login must succeed: expected 429 to be 200`，2026-08-31 04:44-04:51 实测 5 连跑 2 败，窗口过期后 05:23 复跑 101 文件/1054+1 全绿）。判别法：失败必为 429 且集中在该套件；处置：等 15 分钟或减少连跑/手动登录次数，不是代码回归）。**round-150 登记第三类环境性失败**：全量并发下的 DB 集成抖动——`timeseries.test.ts`（500≠200）与 `mapeTracking.monthly.test.ts`（verified 0≠≥1）在并行满载跑中偶发、单跑即绿（2026-08-31 实测各 1-2 次，复跑全绿 1072+1）；判别法：失败套件与改动无关 + 单跑通过 + 换个失败套件的同类复发。测试数随时间变化，运行 `cd backend && pnpm test`、`cd frontend && pnpm test`、`cd inference-service && pytest -q` 获取当前数。
