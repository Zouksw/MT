...
|---|---|---|---|---|
| backend | vitest 4（round-90 从 3 升级） | vitest.config.ts | 96（2026-08-22） | **990 pass / 1 skip** |
...
> 三者合计 **1368 全绿**（990 + 314 + 64，截至 2026-08-22 round-119 权衡批实测）。测试数随时间变化，运行 `cd backend && pnpm test`、`cd frontend && pnpm test`、`cd inference-service && pytest -q` 获取当前数。
