...
|---|---|---|---|---|
| backend | vitest 4（round-90 从 3 升级） | vitest.config.ts | 96（2026-08-23） | **999 pass / 1 skip** |
| frontend | jest 29 + Testing Library | jest.config.js | 33（2026-08-23） | **314 pass** |
| inference | pytest 8 | conftest.py | 4（2026-08-22） | **64 pass** |
| frontend E2E | Playwright | playwright.config.ts | 10 specs | chromium only |

> 三者合计 **1377 全绿**（999 + 314 + 64，截至 2026-08-23 round-120 快修批实测）。测试数随时间变化，运行 `cd backend && pnpm test`、`cd frontend && pnpm test`、`cd inference-service && pytest -q` 获取当前数。
