...
> ## 第三波 v3.0.0（2026-08-30，round-134 规划）— 深探牧集后的开发方向
>
> **依据**：牧集深探新证据（[COMPETITIVE-ANALYSIS §八](COMPETITIVE-ANALYSIS-MOOKET.md)，SPA 路由图/报盘词汇/公司事实）+ MT 现状实测（2026-08-30：汇率组 08-29 日更、活牛/架子牛期货 08-28、IMF 牛肉月度 07/331.78 USC/lb、market_news 90 条、用户 3、backend 1004+1 / frontend 324 全绿、月度预测循环已闭合）。
> **定位重申（不变）**：进口牛肉国际行情 × 可验证 AI 预测。深探进一步证实不与牧集拼国内现货撮合（其 web 端交易面 ~20 条路由 vs 行情 2 条，IM/店铺/金融全套基建——复制成本高且碰红线）。
>
> ### V3-一、方向总纲
>
> **不变前提 P0（用户侧，工程侧已就绪）**：数据解冻三件套——4 个空 API key（MLA/USDA_MARS/OPENWEATHER/FAO）、beef_cut_prices CSV 周更导入（runbook 已备）、种子用户 3→10。以下批次不替代 P0，但**全部使用当前已活着的数据**（汇率/CME 期货/IMF 基准/prediction_logs），零外部依赖即可开工。
>
> **三条战略主线**（证据 → 方向）：
> 1. **把"活着的数据"变成用户可见的价值面**——进口成本计算器（批 1）：牧集结构性做不了（无国际序列+模型），MT 全部输入已在库且日更。
> 2. **补公开中文行情面，吃长尾流量**（批 2）：牧集 SEO 长尾词打法（§8.2）证明该渠道有效；MT 用真数据自动摘要应战（人力研报拼不过也不该拼）。
> 3. **数据词汇对齐行业"普通话"**（批 3）：报盘五维（饲养/天数/VL/品种/仓位，§8.3）是贸易商的阅读习惯；先备好 schema 与导入通道，数据解冻即接上。
>
> ### V3-二、批次总览
>
> | 批 | 内容 | 价值映射 | 规模 | 外部依赖 |
> |----|------|---------|------|---------|
> | 1 | 进口成本计算器 `/tools/landing-cost` | 深探含义 1（差异化工具） | ~1 天 | 无（数据全活） |
> | 2 | 公开中文行情摘要页 + 每周自动数据周报 | 深探含义 4（SEO/分发） | ~1 天 | 无 |
> | 3 | BeefCutPrice 贸易词汇五维 + CSV 模板扩展 | 深探含义 3（数据 schema 对齐） | ~1.5 天（含类型扩展） | 展示价值依赖数据解冻 |
> | 4 | track-record 周度快照物料自动生成 | 分析 P1 公信力运营 | ~0.5 天 | 无 |
> | 5 | 工程卫生打包（登记决策项处置建议） | TECH-DEBT §十四-十六 收尾 | ~0.5 天 | D2-D4 需用户点头 |
>
> **建议顺序**：批 1 → 批 2 → 批 3 → 批 4 → 批 5（先建可见差异化，再备数据词汇，最后收尾）。每批门禁照旧：tsc + 全量测试（数不回退）+ build + PM2 + live 验证 + 独立 commit。
>
> ### V3-三、批次详情
>
> **批 1 — 进口成本计算器（唯一"牧集做不了"的工具）**
> 新公开页 `/tools/landing-cost`（`middleware.ts` PUBLIC_PATHS 增补）：输入出口国（巴西/阿根廷/澳大利亚/美国…）+ 品类基准 → 自动取库内实时数据（出口国汇率 brl/aud/usd_cny、IMF 全球牛肉月度基准或 CME 活牛期货）× 用户可调参数（关税率默认 12%、增值税 9%、代理费、运费系数）→ 输出 **RMB/kg 到岸成本参考区间**。后端 1 个公开聚合端点（cacheRoute 300s + 白名单序列，复用 highlights 基建模式）；前端表单+结果卡。**红线安全**：纯信息计算，非报价非撮合；输出标注"参考估算，非成交价"与数据日期。**诚实门**：任一输入序列过期（超 staleness 窗）→ 显示数据日期与降级提示，不静默用旧值冒充现值。验收：未登录可访问；输入巴西 + 默认参数 → 区间与手算一致（单测钉住公式）；汇率/基准日期可见。
>
> **批 2 — 公开中文行情摘要页（真数据 SEO 面）**
> 新公开页 `/market/digest`（或并入现有公开层）：自动生成的中文行情摘要——全球牛肉基准（月度 MoM）、活牛/架子牛期货（日更）、主要出口国汇率、本周预测对错滚动记录，全部由库内真实数据驱动 + 每条带数据日期。配套：每周 cron 生成 markdown 周报 artifact（可直接分发公众号/社群）。**与牧集的差别写进页面本身**：每行数字可溯源（链接到 track-record/数据源页），不打"分析师洞察"。验收：未登录 200；无 cookie curl 内容含当周日期与真实数字；周报 artifact 生成并 git 可追踪。
>
> **批 3 — 贸易词汇五维（数据 schema 对齐）**
> `BeefCutPrice` 增可空维度：`feedingRegime`（谷饲/草饲）、`feedingDays`（100D+）、`leanPct`（VL 瘦肉率）、`breed`（安格斯…）、`locationPort`（仓位/港口）——倾向先落 `metadata` JSON 扩展 + Prisma 类型（避免重迁移），查询/展示需求出现后再提列。CSV 模板扩展（`GET /api/beef/import/template` 加 5 可选列 + 校验）+ `/beef` 价格表与部位详情页展示（有值才显示，无值不留空列）+ taxonomy 映射表补"报盘俗名→cutCode"（眼肉盖→ribeye cap 类；现 74 部位 4 语言 taxonomy 为基座）。验收：导入含五维 CSV → API 返回带维度 → 前端展示；不含五维的旧模板导入不受影响（向后兼容测试）。
>
> **批 4 — 预测档案周度快照（公信力运营物料）**
> `scripts/generate-track-record-snapshot.ts`：每周从 `getAllModelAccuracy` + 最近 verified 预测生成带时间戳的 markdown 快照（模型榜 + 本周到期预测的对错清单 + 数据覆盖声明）。产物入 git（`docs/track-record/YYYY-WW.md`）——**预测产品的信任货币是事前留档**，快照本身即分发物料。验收：手跑生成当周快照；数字与 `/ai/track-record` live 一致。
>
> **批 5 — 工程卫生打包（全部为已登记项的处置建议，执行以用户点头为准）**
> 见 V3-四 决策项 D2-D4；本批只做无需点头的部分：TECH-DEBT 状态同步、CHANGELOG、（若 D2 批准）休眠表迁移与删除。
>
> ### V3-四、决策项（不擅动，需用户点头）
>
> | # | 事项 | 建议 | 来源 |
> |---|------|------|------|
> | D1 | 批 1-3 三个新方向面是否写入 PRODUCT-SPEC（§五/§七 增补） | 建议：批 1/2 写入（纯增量、不碰"明确不做"）；批 3 作为数据层能力登记 | 本文 + 分析 §八 |
> | D2 | 休眠表清理：`SecurityAuditLog`（49 行）/`ForecastingModel`/`Forecast`（D6 删除后无代码引用，schema+数据保留中） | 建议：一次性迁移删除三模型 + 表（数据先行备份）；或维持登记 | TECH-DEBT round-132 |
> | D3 | portfolios 路由组 + `/api/inference/predict/batch` 孤儿登记 | 建议：维持登记（低优先，删除收益小） | TECH-DEBT §十四/十五 |
> | D4 | `predictionBeefCoverage24h` 对月度节律的结构性眨眼（0/1 跳变） | 建议：指标口径改为"beef 覆盖（不限 24h）+ 最新预测时间戳" | TECH-DEBT round-132 |
>
> ### V3-五、不做清单（继承 + 深探证据扩充）
>
> 继承 v2/分析全部条目。**新增（§8.1 路由证据）**：IM 聊天（TencentCloudChat 类）、商家店铺/入驻体系、报盘/求购大厅、保理/金融/信用/单证面（SK 系统 web 面）、B2B 多角色子账号——以上为牧集 ~20 条路由的运营重镇，全部属"重运营 + 交易语义 + 红线冲突"，明确不做。国内现货报盘采价网络维持不做（分析 §五 结论）。
