...
| [RESEARCH-BEEF-TRADE-DATA-SOURCES.md](RESEARCH-BEEF-TRADE-DATA-SOURCES.md) | **牛肉外贸（海关/贸易流）数据源研究报告**（v2.0.0，2026-09-07 round-160 深挖扩版）：全部关键源本机 live 取证——UN Comtrade 免 key 镜像实测（巴西 t-1/澳新美 t-2、中国年度分国别、HS6 粒度）、单一窗口厂号注册查询可达性（需实名会话）、stats.customs.gov.cn 需中国出口、商业提单库价格带、牧集搜索能力四层拆解与 P0-P2 落地路线；**v2.0.0 六块新增**：国内现货免费面（肉交所带厂号件套价/Mysteel 牛羊业日更 SSR）、Eurostat Comext 欧盟月度镜像（免 key 打通 €645,967 实证）、OECD-FAO 年度基线直链、厂号名录扩展（foodmate GACC 镜像/MPI/MGAP）、运价指数免费面（WCI 文本值/FBX）、提单免费档；俄罗斯月度官方路径全灭确证 |
| [DATA-SOURCES-EVALUATION.md](DATA-SOURCES-EVALUATION.md) | **牛肉外贸数据规模/密度评估——对标牧集的取数决策**（v1.1.0，2026-09-06，round-157）：牧集密度三层拆解（现货日更层无公开源=运营/商业决策）+ MT 数据层 09-06 全量实测基线（18 源仅 5 产数、部位级冻结 04-30）+ 一周内四项源站新变化（USDA mnreports 迁 eWAPS 断供回归、INAC 域名复活、IPCVA 改版、封锁复现）+ 逐层"从哪里获取"决策表与 P0-P2 路线；§八增补批：政府官方 API 扩容实测（NASS/FAS 网关/IBGE SIDRA/MAPA 四个新接口）与牧集/必孚产品官方构建可行性表、本环境不可达清单 |
