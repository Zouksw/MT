...
| [DATA-SOURCES-EVALUATION.md](DATA-SOURCES-EVALUATION.md) | **牛肉外贸数据规模/密度评估——对标牧集的取数决策**（v1.0.0，2026-09-06，round-157）：牧集密度三层拆解（现货日更层无公开源=运营/商业决策）+ MT 数据层 09-06 全量实测基线（18 源仅 5 产数、部位级冻结 04-30）+ 一周内四项源站新变化（USDA mnreports 迁 eWAPS 断供回归、INAC 域名复活、IPCVA 改版、封锁复现）+ 逐层"从哪里获取"决策表与 P0-P2 路线；TradeSources 落地一周后的复盘视角 |
