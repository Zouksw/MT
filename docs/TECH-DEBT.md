...
- **round-132 登记——`predictionBeefCoverage24h` 对月度节律结构性眨眼（0/1 指标）**：该健康检查计数"beef_cuts 类目商品 24h 内有 prediction_logs 行"——由于 cut 序列用合成 `cut:` 键不入 commodities 表，实际唯一能推动它的是 `beef_carcass_us`（月度）。月度序列一月只落一轮日志（ADR-0001 ④），故该指标在两次发布之间的 ~29 天里诚实但误导地读 0（2026-08-30 实测：末轮日志 08-24 → 0；预测缓存本身持续保温）。若要有意义需改为"cadence 感知窗口（月度行按 90d）"或"缓存温暖度"语义，牵动 health 路由类型与键名，单列轮次再动。
