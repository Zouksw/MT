...
5. **订阅谓词（cadence 感知）**：daily = 现行（7 天窗内 ≥2 点）；monthly = 最新点 ≤90 天 且 全序列 ≥3 点（阈值取自 `cadence.ts` 策略模块，防月内订阅抖动）。
   - **修订 round-132（2026-08-30，60→90）**：原值 60 天 = "2× 发布节奏"，混淆了发布节奏与点龄。实测（`commodity_prices.created_at − date`）：月度点首发滞后 44–53 天（点日期 M-01，发布于 M+1 中下旬），故健康序列的最新点龄跨周期从 ~45 天长到 ~76 天——60 天窗在每周期后半段会把全部健康月度序列误判 stale（live 实证：`beef_carcass_us` 最新点 2026-07-01 于 08-30 达 60.2 天，源健康（下次发布 ~09 中旬），调度器当日把 6 条健康月度序列全部拒之门外；若不修，Pass A/B 清扫也会在 >60 天时把它们误冻结）。90 = 45 滞后 + 31 节奏 + ~2 周滑期余量；观测到的 83–114 天"滞后"是对已发布点的回填追赶，非最新点龄。**形式语义不变（"≤ staleness 窗口"），仅修正常量值**；一处常量，调度谓词 / Pass A 宽限 / Pass B 窗口 / expire 宽限 / freshness 板五处同步生效。
